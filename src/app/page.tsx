...
import {
  ...
  LogOut, UserCheck, Activity, Lock, Package,
  FileText, ChevronRight, Database, Wrench, Filter, IndianRupee
} from "lucide-react";
...
import { getPendingUsersCount } from "@/app/actions/userActions";

export default function DashboardPage() {
  // Start with loading = true to prevent UI flashing
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      // 1. Fetch Session (Cookies) & Stats (Cache) at the exact same time
      ...
      setStats(statsData);
      setCycle(cycleData.stats || { year1: 0, year2: 0, year3: 0, newlyVerified: 0 });
      setSession(sessionData);

      // 2. If Super Admin, fetch the pending users count
      if (sessionData?.role === "SUPER_ADMIN") {
        const pendingRes = await getPendingUsersCount();
        if (pendingRes.success) setPendingUserCount(pendingRes.count);
      }
    } catch (error) {
  ...
  if (loading) return <NavigationLoader message="Syncing Dashboard..." />;
      ...
      <main className="flex-1 px-6 pb-6 space-y-8 overflow-y-auto pt-4">
        ...
        <section className="bg-white dark:bg-gray-900 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-800">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-gray-900 dark:text-white uppercase tracking-wider text-xs">3-Year Cycle</h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-y-3 gap-x-6">
            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-500" /><span className="text-[11px] font-medium text-gray-500">Year 1: <strong>{cycle.year1}</strong></span></div>
            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-blue-500" /><span className="text-[11px] font-medium text-gray-500">Year 2: <strong>{cycle.year2}</strong></span></div>
            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-purple-600" /><span className="text-[11px] font-medium text-gray-500">Year 3: <strong>{cycle.year3}</strong></span></div>
            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-gray-300" /><span className="text-[11px] font-medium text-gray-500">New: <strong>{cycle.newlyVerified}</strong></span></div>
          </div>
        </section>
        ...
        {hasAdminAccess && (
          <section className="animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {isSuperAdmin && (
                <button onClick={() => handleBack("/admin/users")} className="relative flex flex-col p-4 bg-white dark:bg-gray-900 rounded-[2rem] border border-blue-100 dark:border-blue-900/30 shadow-sm active:scale-95 transition-all text-left">
                  {pendingUserCount > 0 && (
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/50 p-3 rounded-2xl w-fit mb-3"><UserCheck className="w-6 h-6 text-blue-600" /></div>
              ...
              <button onClick={() => handleBack("/admin/inventory")} className="flex flex-col p-4 bg-white dark:bg-gray-900 rounded-[2rem] border border-orange-100 dark:border-orange-900/30 shadow-sm active:scale-95 transition-all text-left">
                <div className="bg-orange-50 dark:bg-orange-900/50 p-3 rounded-2xl w-fit mb-3"><Package className="w-6 h-6 text-orange-600" /></div>
              ...
              <button onClick={() => handleBack("/admin/reports")} className="flex flex-col p-4 bg-white dark:bg-gray-900 rounded-[2rem] border border-green-100 dark:border-green-900/30 shadow-sm active:scale-95 transition-all text-left">
                <div className="bg-green-50 dark:bg-green-900/50 p-3 rounded-2xl w-fit mb-3"><FileText className="w-6 h-6 text-green-600" /></div>
              ...
              {isSuperAdmin && (
                <button onClick={() => handleBack("/admin/audit")} className="flex flex-col p-4 bg-white dark:bg-gray-900 rounded-[2rem] border border-purple-100 dark:border-purple-900/30 shadow-sm active:scale-95 transition-all text-left">
                  <div className="bg-purple-50 dark:bg-purple-900/50 p-3 rounded-2xl w-fit mb-3"><Activity className="w-6 h-6 text-purple-600" /></div>
              ...
              {isSuperAdmin && (
                <button onClick={() => handleBack("/admin/advanced-tools")} className="col-span-2 flex flex-col p-4 bg-gradient-to-br from-emerald-900 via-green-900 to-emerald-950 rounded-[2rem] shadow-lg active:scale-95 transition-all text-left border border-emerald-500/30">
                  <div className="bg-emerald-400/10 p-3 rounded-2xl w-fit mb-3 border border-emerald-400/20"><Wrench className="w-6 h-6 text-emerald-400" /></div>
                  <h3 className="font-black text-emerald-50 text-sm">Advanced Admin Tools</h3>
                  <p className="text-[10px] text-emerald-200/60 mt-1 uppercase font-bold tracking-tighter">Bulk Import • Direct Overrides • System Fixes</p>
                </button>
              )}
              ...
              {isSuperAdmin && (
                <button onClick={() => handleBack("/admin/migrate")} className="col-span-2 flex flex-col p-4 bg-gradient-to-br from-gray-900 to-gray-800 dark:from-gray-800 dark:to-black rounded-[2rem] shadow-md active:scale-95 transition-all text-left border border-gray-700">
                  <div className="bg-white/10 p-3 rounded-2xl w-fit mb-3"><Database className="w-6 h-6 text-white" /></div>
              ...
              {/* --- BAITULMAL COLLECTION CARD --- */}
              {session?.hasCollectionAccess && (
                <button onClick={() => handleBack("/admin/collections")} className="col-span-2 flex items-center justify-between p-4 bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/10 rounded-[2rem] shadow-sm active:scale-95 transition-all text-left border border-yellow-200 dark:border-yellow-900/30">
                  <div className="flex items-center gap-4">
                    <div className="bg-gradient-to-br from-yellow-400 to-amber-600 p-3 rounded-2xl w-fit shadow-md"><IndianRupee className="w-6 h-6 text-white" /></div>
                    <div>
                      <h3 className="font-black text-gray-900 dark:text-white text-sm">Baitulmal Collections</h3>
                      <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 uppercase font-bold tracking-tighter">Donations • Receipts • Approvals</p>
                    </div>
                  </div>
        ...
        <section>
          ...
          <div className="space-y-3">
            <button onClick={() => handleBack("/register")} className="w-full flex items-center p-4 bg-white dark:bg-gray-900 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-800 active:scale-[0.98] transition-transform duration-100 group text-left">
              <div className="bg-green-50 dark:bg-green-900/30 p-3 rounded-xl mr-4 group-hover:bg-green-100 transition-colors"><UserPlus className="w-6 h-6 text-green-600 dark:text-green-400" /></div>
              <div className="flex-1"><h3 className="font-bold text-base text-gray-900 dark:text-gray-100">Register Beneficiary</h3><p className="text-xs text-gray-500 dark:text-gray-400">Add new family</p></div>
            ...
            <button onClick={() => handleBack("/verify")} className="w-full flex items-center p-4 bg-white dark:bg-gray-900 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-800 active:scale-[0.98] transition-transform duration-100 group text-left">
              <div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-xl mr-4 group-hover:bg-blue-100 transition-colors"><Search className="w-6 h-6 text-blue-600 dark:text-blue-400" /></div>
              <div className="flex-1"><h3 className="font-bold text-base text-gray-900 dark:text-gray-100">Verify / Search</h3><p className="text-xs text-gray-500 dark:text-gray-400">Check status & details</p></div>
              <ChevronRight className="w-5 h-5 text-gray-300" />
            </button>
            ...
            <button onClick={() => handleBack("/beneficiaries")} className="w-full flex items-center p-4 bg-white dark:bg-gray-900 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-800 active:scale-[0.98] transition-transform duration-100 group text-left">
              <div className="bg-purple-50 dark:bg-purple-900/30 p-3 rounded-xl mr-4 group-hover:bg-purple-100 transition-colors"><Users className="w-6 h-6 text-purple-600 dark:text-purple-400" /></div>
              <div className="flex-1"><h3 className="font-bold text-base text-gray-900 dark:text-gray-100">View All Beneficiaries</h3><p className="text-xs text-gray-500 dark:text-gray-400">Master list & edits</p></div>
              <ChevronRight className="w-5 h-5 text-gray-300" />
            </button>

            {hasAdminAccess && (
              <button onClick={() => handleBack("/admin/smart-explorer")} className="w-full flex items-center p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-[2rem] shadow-sm border border-indigo-200 dark:border-indigo-800/50 active:scale-[0.98] transition-transform duration-100 group text-left mt-2">
                <div className="bg-indigo-100 dark:bg-indigo-800 p-3 rounded-xl mr-4 group-hover:bg-indigo-200 transition-colors"><Filter className="w-6 h-6 text-indigo-600 dark:text-indigo-400" /></div>
                <div className="flex-1"><h3 className="font-bold text-base text-indigo-900 dark:text-indigo-100">Smart Explorer</h3><p className="text-[11px] text-indigo-600 dark:text-indigo-400 font-bold uppercase tracking-wider mt-0.5">Filter by Year, Area & Ref</p></div>
        ...
        <section className="pb-10">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => handleBack("/distribution/check-in")} className="flex flex-col h-full p-4 bg-orange-50 dark:bg-orange-900/10 rounded-[2rem] border border-orange-100 dark:border-orange-900/30 active:scale-[0.98] transition-transform text-left">
              <div className="bg-white dark:bg-orange-900/30 p-3 rounded-xl w-fit mb-3 shadow-sm"><ShieldCheck className="w-6 h-6 text-orange-600 dark:text-orange-400" /></div>
            ...
            <button onClick={() => handleBack("/distribution/live-queue")} className="flex flex-col h-full p-4 bg-teal-50 dark:bg-teal-900/10 rounded-[2rem] border border-teal-100 dark:border-teal-900/30 active:scale-[0.98] transition-transform text-left">
              <div className="bg-white dark:bg-teal-900/30 p-3 rounded-xl w-fit mb-3 shadow-sm"><ListOrdered className="w-6 h-6 text-teal-600 dark:text-teal-400" /></div>
